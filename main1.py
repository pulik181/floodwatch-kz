...
def run_health_check_server():
  """Запуск фонового веб-сервера, чтобы Render успешно прошёл проверку порта"""
...
LOCATIONS = [
    ...
    {
        "name": "Медеу (Алматы, КЗ)",
        "lat": 43.1575,
        "lon": 77.0589,
        "elevation": 1690,
    },
    {
        "name": "Талгар (Ущелье, КЗ)",
        "lat": 43.2500,
        "lon": 77.2300,
        "elevation": 1200,
    },
...
MESSAGES = {
    "kz": {
        ...
        "select_lang_prompt": (
            "👋 Қош келдіңіз! Мәтіндер қай тілде көрсетілсін? Тілді таңдаңыз:"
        ),
        ...
        "alert_title": (
            "🚨 <b>СППР FloodWatch KZ АВТОМАТТЫ ДАБЫЛ СИГНАЛЫ</b>\n\n"
        ),
        "alert_high": (
            "🚨 <b>КРИТИКАЛЫҚ ҚАУІП</b>: <b>{name}</b> нүктесінде ({temp}°C,"
            " {rain} мм жауын-шашын)!"
        ),
        "alert_med": (
            "⚠️ <b>НАЗАР АУДАРЫҢЫЗ</b>: <b>{name}</b> нүктесінде қауіп деңгейі"
            " көтерілді ({temp}°C, {rain} мм)."
        ),
        "help": (
            "ℹ️ <b>ШЕШІМ ҚАБЫЛДАУДЫ ҚОЛДАУ ЖҮЙЕСІ (FloodWatch KZ)</b>\n\nЖүйе"
            " тау бөктері мен қауіпті аймақтарда үздіксіз фондық мониторинг"
            " жүргізеді.\n\n• Мәлімет алу үшін <b>📊 Ауа райы статусы</b>"
            " батырмасын басыңыз.\n• Автоматты тексеру әр 15 минут сайын"
    ...
    "ru": {
        ...
        "select_lang_prompt": (
            "👋 Добро пожаловать! Выберите удобный язык интерфейса:"
        ),
        ...
        "alert_title": (
            "🚨 <b>АВТОМАТИЧЕСКИЙ СИГНАЛ ТРЕВОГИ FloodWatch KZ</b>\n\n"
        ),
        "alert_high": (
            "🚨 <b>КРИТИЧЕСКАЯ УГРОЗА</b>: В точке <b>{name}</b> ({temp}°C,"
            " {rain} мм осадков)!"
        ),
        "alert_med": (
            "⚠️ <b>ВНИМАНИЕ</b>: В точке <b>{name}</b> повышен уровень риска"
            " ({temp}°C, {rain} мм)."
        ),
        "help": (
            "ℹ️ <b>СИСТЕМА ПОДДЕРЖКИ ПРИНЯТИЯ РЕШЕНИЙ (FloodWatch KZ)</b>\n\nСистема"
            " выполняет непрерывный фоновый мониторинг метеорологических"
            " сеток в предгорных и опасных зонах.\n\n• Нажмите <b>📊 Статус"
            " погоды</b> для мгновенного сбора данных по всем точкам.\n•"
    ...
    "en": {
        ...
        "error_fetch": (
            "⚠️ Failed to fetch weather data. Please try again later."
        ),
        ...
        "alert_high": (
            "🚨 <b>CRITICAL RISK</b>: At <b>{name}</b> ({temp}°C, {rain} mm"
            " precipitation)!"
        ),
        "alert_med": (
            "⚠️ <b>WARNING</b>: Elevated risk level at <b>{name}</b> ({temp}°C,"
            " {rain} mm)."
        ),
        "help": (
            "ℹ️ <b>DECISION SUPPORT SYSTEM (FloodWatch KZ)</b>\n\nThe system"
            " continuously performs background monitoring of meteorological"
            " grids in foothill and high-risk zones.\n\n• Press <b>📊 Weather"
            " Status</b> for instant data collection across all points.\n•"
            " Automatic checks run every 15 minutes.\n• Alerts are sent only"
            " when an actual risk occurs."
        ),
    },
}
...
def save_to_csv(row: list):
  ...
  try:
    with open(CSV_FILE, "a", newline="", encoding="utf-8") as f:
      ...
      if not file_exists:
        writer.writerow(
            ["Дата_Время", "Локация", "Температура_C", "Осадки_мм", "Уровень_Риска"]
        )
      writer.writerow(row)
  except Exception as e:
...
def get_almaty_time() -> str:
  """Расчёт точного времени Алматы (UTC+5)"""
...
def generate_manual_report(lang: str):
  ...
  for place in LOCATIONS:
    temp, rain = get_current_weather(
        place["lat"], place["lon"], place.get("elevation")
    )
  ...
  if raw_alerts:
    for chat_id in subscribers:
      ...
      for alert_type, name, temp, rain in raw_alerts:
        if alert_type == "high":
          user_lines.append(
              msg["alert_high"].format(name=name, temp=temp, rain=rain)
          )
        else:
          user_lines.append(
              msg["alert_med"].format(name=name, temp=temp, rain=rain)
          )
      ...
      if res.get("ok") and res.get("result"):
        for update in res["result"]:
          ...
          if "message" in update and "text" in update["message"]:
            ...
            logging.info(
                f"Команда '{text}' от chat_id: {chat_id} (Язык: {lang})"
            )

            if text == "/start" or text in [
                "🌐 Сменить язык",
                "🌐 Тілді ауыстыру",
                "🌐 Change Language",
                "/lang",
