...
# ============================================
# 2. МУЛЬТИЯЗЫЧНЫЙ СЛОВАРЬ (KZ / RU / EN)
# ============================================
MESSAGES = {
    "kz": {
        ...
        "polling": "⏳ Open-Meteo метеостанцияларынан мәліметтер жиналуда...",
        "header": "🌊 *СППР FloodWatch KZ: АҒЫМДАҒЫ МОНИТОРИНГ*\n",
        "time_prefix": "🕒 _Тексеру уақыты: ",
        "risk_labels": {
        ...
        "alert_title": "🚨 *СППР FloodWatch KZ АВТОМАТТЫ ДАБЫЛ СИГНАЛЫ*\n\n",
        "alert_high": "🚨 *КРИТИКАЛЫҚ ҚАУІП*: *{name}* нүктесінде ({temp}°C, {rain} мм жауын-шашын)!",
        "alert_med": "⚠️ *НАЗАР АУДАРЫҢЫЗ*: *{name}* нүктесінде қауіп деңгейі көтерілді ({temp}°C, {rain} мм).",
        "help": (
            "ℹ️ *ШЕШІМ ҚАБЫЛДАУДЫ ҚОЛДАУ ЖҮЙЕСІ (FloodWatch KZ)*\n\n"
            "Жүйе тау бөктері мен қауіпті аймақтарда үздіксіз фондық мониторинг жүргізеді.\n\n"
            "• Мәлімет алу үшін *📊 Ауа райы статусы* батырмасын басыңыз.\n"
    ...
    "ru": {
        ...
        "polling": "⏳ Опрос метеостанций Open-Meteo...",
        "header": "🌊 *СППР FloodWatch KZ: ТЕКУЩИЙ МОНИТОРИНГ*\n",
        "time_prefix": "🕒 _Время проверки: ",
        "risk_labels": {
        ...
        "alert_title": "🚨 *АВТОМАТИЧЕСКИЙ СИГНАЛ ТРЕВОГИ FloodWatch KZ*\n\n",
        "alert_high": "🚨 *КРИТИЧЕСКАЯ УГРОЗА*: В точке *{name}* ({temp}°C, {rain} мм осадков)!",
        "alert_med": "⚠️ *ВНИМАНИЕ*: В точке *{name}* повышен уровень риска ({temp}°C, {rain} мм).",
        "help": (
            "ℹ️ *СИСТЕМА ПОДДЕРЖКИ ПРИНЯТИЯ РЕШЕНИЙ (FloodWatch KZ)*\n\n"
            "Система выполняет непрерывный фоновый мониторинг метеорологических сеток "
            "в предгорных и опасных зонах.\n\n"
            "• Нажмите *📊 Статус погоды* для мгновенного сбора данных по всем точкам.\n"
    ...
    "en": {
        ...
        "polling": "⏳ Querying Open-Meteo weather stations...",
        "header": "🌊 *DSS FloodWatch KZ: CURRENT MONITORING*\n",
        "time_prefix": "🕒 _Check time: ",
        "risk_labels": {
            "HIGH_RISK": "🚨 HIGH RISK",
        ...
        "alert_title": "🚨 *AUTOMATIC ALERT FloodWatch KZ*\n\n",
        "alert_high": "🚨 *CRITICAL RISK*: At *{name}* ({temp}°C, {rain} mm precipitation)!",
        "alert_med": "⚠️ *WARNING*: Elevated risk level at *{name}* ({temp}°C, {rain} mm).",
        "help": (
            "ℹ️ *DECISION SUPPORT SYSTEM (FloodWatch KZ)*\n\n"
            "The system continuously performs background monitoring of meteorological grids "
            "in foothill and high-risk zones.\n\n"
            "• Press *📊 Weather Status* for instant data collection across all points.\n"
...
def generate_manual_report(lang: str):
    ...
    for place in LOCATIONS:
        ...
        lines.append(f"• *{place['name']}*: {t_str} | {r_str} ({risk_text})")
        save_to_csv([now_str, place["name"], temp, rain, risk])

    header = msg["header"] + msg["time_prefix"] + f"{now_str}_\n\n"
    return header + "\n".join(lines)
